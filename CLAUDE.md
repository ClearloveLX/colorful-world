...
This is a **local media gallery** for browsing, filtering, and managing images/videos. It has two modes:
...
- **`FileManager`** (`backend/services/file_manager.py`) handles image/video file operations: scanning source directories for media files (by extension), copying files to output/good/recycle_bin folders.
...
### Dependencies
- **Backend** (`requirements.txt`): opencv-python, Pillow, numpy, fastapi, uvicorn[standard], mutagen
